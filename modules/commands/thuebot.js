...
let keysData = readJsonSafe(keysDataPath);
let data = readJsonSafe(thuebotDataPath);
...
exports.handleEvent = async function({ api, event }) {
    ...
    if (event.body && event.body.startsWith('kzi_')) {
        ...
        if (keyIndex !== -1) {
            if (keysData[keyIndex].used) {
                ...
                saveThuebotData();
                saveKeysData();
                api.changeNickname(`[ ${global.config.PREFIX} ] • ${(!global.config.BOTNAME) ? "Made by Bảo" : global.config.BOTNAME} | HSD: ${time_end}`,
...
📌 𝐂𝐨̀𝐧 ${(()=> {
    ...
    o.api.changeNickname(
        `[ ${threadPrefix} ] • ${(!global.config.BOTNAME) ? "Made by kzi" : global.config.BOTNAME} | HSD: ${newEndDate}`, 
        threadIDToUpdate, 
