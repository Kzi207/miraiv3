...
let appState = [];
try {
  const fbstatePath = path.join(__dirname, "../../fbstate.json");
  const raw = fs.existsSync(fbstatePath) ? fs.readFileSync(fbstatePath, "utf-8") : "";
  appState = raw && raw.trim() ? JSON.parse(raw) : [];
} catch (e) {
  appState = [];
}
