...
module.exports.onLoad = async() => {
  ...
  if (!fs.existsSync(bg)) await downloadFile(bglink, path.resolve(bg));
  if (!fs.existsSync(dicon)) await downloadFile(iconlink, path.resolve(dicon));
  if (!fs.existsSync(font)) await downloadFile(fontlink, path.resolve(font));
}
